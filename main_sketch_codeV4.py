...
def add_ruler(plt, ax, length):
    ...
    location = [plt.xlim()[0] + (plt.xlim()[1] - plt.xlim()[0]) * .05+0.0000215,
                plt.ylim()[1] - ((plt.ylim()[1] - plt.ylim()[0]) * (0.05 + (height_scale * .04)))+0.0000009]
...
class Drone:
    ...
    def update_position(self, cf_position, crazyflie):
        ...
        if crazyflie == 1:
            longitude = self.start_lat_lon.longitude + ((cf_position[0]+0.3) / (
                    math.cos(self.start_lat_lon.latitude * 0.01745) * (EARTH_CIRCUMFERENCE / 360)))
        else:
            longitude = self.start_lat_lon.longitude + ((cf_position[0]-0.3) / (
                    math.cos(self.start_lat_lon.latitude * 0.01745) * (EARTH_CIRCUMFERENCE / 360)))
        latitude = self.start_lat_lon.latitude + ((cf_position[1]+0.9) / (EARTH_CIRCUMFERENCE / 360))
...
def run_sketch(df1p, df2p, offset, lambda_value, plumes, threshold, iterations, base_file_name=None,
    ...
    configs = [
        {'drone_id': drone1_id, 'starting_position': [-0.3, -0.9, 1.0]},
        {'drone_id': drone2_id, 'starting_position': [0.3, -0.9, 1.0]}
    ]
    ...
    plot_plumes(plt, threshold, lon=[-106.59725, -106.59714], lat=[35.19725, 35.19736], plumes=plumes)
    
    '''
    rtl_boundary = [[-106.59716, 35.19727],
                    [-106.59716, 35.197325],
                    [-106.59723, 35.197325],
                    [-106.59723, 35.19727],
                    [-106.59716, 35.19727]]
    ...
    if base_file_name is not None:
        plt.savefig(f'{base_file_name}_python_sim_small3.pdf', format="pdf", dpi=300)

    for drone in drones:
        velocities = drone.velocity
        t_velocities = drone.t_velocity
        des_velocities = drone.des_velocity
        t_des_velocities = drone.t_des_velocity
        if drone.drone_id == drone1_id:
            plt.figure(2, figsize=(10, 8))
            plt.title("Crazyflie 1")
        else:
            plt.figure(3, figsize=(10, 8))
            plt.title("Crazyflie 2")
        plt.plot(t_des_velocities, des_velocities[:, 0], label="Desired V_x")
        plt.plot(t_des_velocities, des_velocities[:, 1], label="Desired V_y")
        plt.plot(t_velocities, velocities[:, 0], label="Measured V_x")
        plt.plot(t_velocities, velocities[:, 1], label="Measured V_y")
        plt.grid()
        plt.legend()
    
    
    # Base directory where all files will be saved
    base_directory = "/home/marhes/Kevin/VOLCAN/FIGURES_PAPER"
    ...
    experiment_directory = os.path.join(base_directory, f"experiment_{experiment_timestamp}")

    # Ensure the experiment directory exists
    os.makedirs(experiment_directory, exist_ok=True)

    filename = os.path.join(experiment_directory, f"drone1_latitude_trajectory_{experiment_timestamp}.csv")

    df = pd.DataFrame(df1latitudes)
    df.to_csv(filename, index=False)

    filename = os.path.join(experiment_directory, f"drone2_latitude_trajectory_{experiment_timestamp}.csv")

    df = pd.DataFrame(df1latitudes)
    df.to_csv(filename, index=False)

    filename = os.path.join(experiment_directory, f"drone1_longitude_trajectory_{experiment_timestamp}.csv")

    df = pd.DataFrame(df1longitudes)
    df.to_csv(filename, index=False)

    filename = os.path.join(experiment_directory, f"drone2_longitude_trajectory_{experiment_timestamp}.csv")
...
def main(args=None) -> None:
    ...
    df1p = LatLon(35.1972819, -106.5971775)
    df2p = LatLon(35.1972819, -106.5971755)

    run_sketch(df1p, df2p, 0.17, 0.008, plumes, threshold, iterations=500, base_file_name='single_plume')
